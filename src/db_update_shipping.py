...
CREATE OR REPLACE FUNCTION public.sp_get_shipping()
 RETURNS TABLE(address text, username text, name text, color text, size text, paymethod text, price numeric, cost numeric, totalamount numeric, status text, image text, id_status integer, id_order integer, quantityorders integer, custom_image text)
 LANGUAGE sql
 STABLE
AS $function$
    SELECT
